...
B1=fit(pass~.,math[,c(inputs,bout)],model="rpart",task="class") # fit a decision tree
# print(B1@object)
cat("DT Acc:")
showres(B1,cmath,"pass")

B2=fit(pass~.,math[,c(inputs,bout)],model="ctree",task="class") # fit a conditional inference tree
# print(B2@object)
cat("\nConditional Inference Tree Acc:")
showres(B2,cmath,"pass")

B3=fit(pass~.,math[,c(inputs,bout)],model="mlpe",task="class") # fit a multilayer perceptron ensemble
# print(B3@object)
cat("\nMultiple Perceptron ensemble Acc:")
showres(B3,cmath,"pass")

B4=fit(pass~.,math[,c(inputs,bout)],model="ksvm",task="class") # fit a supmatht vector machine
# print(B4@object)
cat("\nSVM Acc:")
showres(B4,cmath,"pass")

B5=fit(pass~.,math[,c(inputs,bout)],model="naive",task="class") # fit a naive bayes
# print(B5@object)
cat("\nNaive Bayes Acc:")
showres(B5,cmath,"pass")

B6=fit(pass~.,math[,c(inputs,bout)],model="knn",task="class") # fit a knn
# print(B6@object)
cat("\nkNN Acc:")
showres(B6,cmath,"pass")

B7=fit(pass~.,math[,c(inputs,bout)],model="bagging",task="class") # fit bagging
# print(B7@object)
cat("\nBagging Acc:")
showres(B7,cmath,"pass")

B8=fit(pass~.,math[,c(inputs,bout)],model="boosting",task="class") # fit boosting
# print(B8@object)
cat("\nBoosting Acc:")
showres(B8,cmath,"pass")

B9=fit(pass~.,math[,c(inputs,bout)],model="randomForest",task="class") # fit a random forest
# print(B9@object)
cat("\nRF Acc:")
showres(B9,cmath,"pass")
sink()
